...
##################################################################################################################            
            ...
            # prüfen und anzeigen , ob Histerese beginnt , endet , läuft           
            TextString = TextString + Func_Hist.pruef (KTakt,KTmin,KThist,'DK2_Hist')
            print (TextString)
            # Prüfung , ob Nachtaufladung Kessel oder alternativ Direktladung einzuschalten ist
            # Direktladung nur , wenn keine Nachtladung läuft
            # Entscheid , ob fǘr Nachtaufladung Kessel eingeschaltet wird
            # Prüfung Plausibilität der Ladezeiten                
            NT_Zeit_Geraet = Func_NT_Ladung.pruef (NT_Zeit_Start,NT_Zeit_Ende,KNvon,KNbis,DTvon,DTbis,DTakt)
            if NT_Zeit_Geraet not in ['ein','aus'] :     # bei Intervall-Fehler Abbruch
                raise AssertionError (' Nachtaufladung Kessel ',NT_Zeit_Geraet)
            # Annahme : weder Nacht- noch Direktladung aktiv --> Relais KK2 im Keller aus
            K2_Relais = 'KK2'
            K2_Schalter = False
            # Nachtaufladung einschalten ?         
            if NT_Zeit_Geraet == 'ein' :        # NT-Ladung Kessel eingeschaltet
                K2_Relais = 'HK2'               # Relais für Nachtladung verwenden
                GVS.RelTab['DK2_Hist'] = False  # bei NT-Ladung keine Histerese
                if KTakt >= KTmax :             
                    K2_Schalter = False         # Schalter aus , wenn max. Temperatur erreicht   
                    print ("-- Nachtladung      nicht aktiv , da max.Temperatur erreicht",KTakt)
                else :
                    K2_Schalter = True          # Schalter ein , wenn max. Temperatur noch nicht erreicht  
                    print ("-- Nachtladung      aktiv von",KNvon,"bis",KNbis,'aktuell',DTakt)
            else :                              # NT-Ladung Kessel ausgeschaltet
                K2_Schalter = False             # Schalter aus , da keine Nachtladung
                print     ("-- Nachtladung      nicht aktiv , nur von",KNvon,"bis",KNbis,'aktuell',DTakt,)
                # Prüfung , ob Histerese läuft
                if GVS.RelTab['DK2_Hist'] :             # Histerese läuft --> 
                    K2_Relais = 'DK2'                   # Relais für Direktladung verwenden
                    if KTakt < (KTmin + KThist) :       # Direktladung einschalten
                        K2_Schalter = True              # Schalter ein , da Direktladung 
                        TextString = "-- Direktladung     aktiv aktuell " + str(KTakt) + " war gefallen unter Minimum " + str(KTmin)
                        TextString = TextString + ' Histerese noch bis ' + str(KTmin + KThist) + ' eingeschaltet'
                        print (Fore.YELLOW + TextString)
                        
                    else :                              # Direktladung ausschalten
                        K2_Schalter = False             # Schalter aus , da Direktladung beendet
                        print     ("-- Direktladung     nicht mehr aktiv , da max.Temperatur erreicht",KTakt)
                else :             
                    print ("-- Direktladung     nicht aktiv , da ausreichende Temperatur ",KTakt)
...
##################################################################################################################            
        ...
        if Warmwasser :
            ...
            # Prüfung Plausibilität der Ladezeiten                
            NT_Zeit_Boiler = Func_NT_Ladung.pruef (NT_Zeit_Start,NT_Zeit_Ende,BNvon,BNbis,DTvon,DTbis,DTakt)
        ...
        #  keine Raumheizung --> alle Relais ausschalten , die davon betroffen sind !           
            ...
            TextString = Fore.YELLOW + 'Raumheizung und zugehörige Relais Boiler WK1 / WK2 / HK2 wie DK2 ausgeschaltet'
            print (TextString)
            ...
            if TextString != () : print (TextString)
            time.sleep(1)                   # 1 sec warten
            RELAIS = 'HK2'  # HK2 = DK2 , gleiches Relais
            TextString = (Func_Relais.Schaltung (RELAIS , Schalter , drucken , loggen))
            if TextString != () : print (TextString)
            time.sleep(1)                   # 1 sec warten
...
#            TextString = Style.BRIGHT + 'Schaltvorgänge Warmwasserbereitung : ' + Style.RESET_ALL 
#            if not Raumheizung :
#                TextString = TextString + "(Hintergrund rot->aus,grün->ein  Schrift weiß->unverändert,schwarz->geschaltet)"
#            print (TextString) 
        ...
        #  Keine Warmwasserbereitung -->  Relais Boiler ausschalten          
            ...
            TextString = Fore.YELLOW + 'Warmwasserbereitung und Relais Boiler BK1 wie DK1 ausgeschaltet' + Style.RESET_ALL 
            print (TextString)
            # Schaltvorgang aus durchführen
            Schalter         = False
            drucken          = True
            loggen           = True
            RELAIS = 'BK1'   # BK1 = DK1 , gleiches Relais
            TextString = (Func_Relais.Schaltung (RELAIS , Schalter  , drucken , loggen))
            if TextString != () : print (TextString)
            time.sleep(1)                   # 1 sec warten

        # Sonstige Schaltvorgänge , nicht Raumheizung oder Warmwasserbereitung
